...
describe("prompt tool gating", () => {
	...
	it("documents the active Execute shell", () => {
		...
		expect(bashPrompt).toContain("Commands run under bash (/bin/bash)");
		expect(bashPrompt).toContain(
			"Exit code 124 means the command was killed on timeout",
		);
	...
	it("includes the todo reminder segment only when provided", () => {
		...
		).toContain("No task list exists yet for this session");
		expect(
			getSystemPrompt({
				profile: "anthropic",
		...
		expect(getSystemPrompt({})).not.toContain(
			"No task list exists yet for this session",
		);
	});
});
