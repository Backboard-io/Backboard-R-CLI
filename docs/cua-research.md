...
**Backboard (SSO) path.** The server lifts `__image_base64` only from the *top level* of a tool output, and both the new Computer payload (`observation.__image_base64`) and the Browser payload (`results[i].__image_base64`, even on `main`) nested it — so Backboard models got base64 text and no picture. `ComputerTool`/`BrowserTool` now hoist the final image to the top level on the wire (`hoistObservationImage` / `hoistBrowserImage`); the structured `data` is unchanged and the BYOK helper handles both shapes. Verified with the grounding eval in `--blind` mode (every element stripped, so only the image can ground the click) on production via SSO: gpt-5.5, xai/grok-4.6, gemini-3.5-flash, claude-sonnet-5, Bedrock claude-fable-5, kimi-k3 (OpenRouter via Backboard), gpt-oss-120b (Cerebras) — **7/7**, 0.4–9k tokens each instead of ~45k. Cohere command-a never calls tools (text-only model). Note: the `cli-eval` `.env` points at `dev2.app.backboard.io`, where the Bedrock route returns "Internal error while generating AI response." on the tool-output leg; production does not.
